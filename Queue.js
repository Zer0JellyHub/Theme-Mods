(function () {
    ...
    function syncAllCardButtons() {
        ...
        document.querySelectorAll('.jfq-add-btn[data-jfq-sr-id]').forEach(function(btn){
            ...
            btn.textContent=inq?'✓ Remove':'+ Add';
        ...
        if(db){
            ...
            if(did){
                ...
                db.title=inq2?'Remove from queue':'Add to queue';
    ...
    function injectCSS() {
        ...
        s.id = CSS_ID;
        s.textContent = [
            '#jfq-overlay{position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.55);backdrop-filter:blur(24px) saturate(1.4);-webkit-backdrop-filter:blur(24px) saturate(1.4);display:flex;flex-direction:column;overflow:hidden;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;}',
            '#jfq-head{display:flex;align-items:center;justify-content:space-between;padding:14px 3.5%;border-bottom:1px solid rgba(255,255,255,.12);flex-shrink:0;background:rgba(0,0,0,.2);}',
            '#jfq-title{font-size:1.2em;font-weight:300;letter-spacing:.03em;display:flex;align-items:center;gap:10px;color:rgba(255,255,255,.95);}',
            '#jfq-title svg{width:20px;height:20px;opacity:.9;}',
            '#jfq-head-right{display:flex;align-items:center;gap:8px;}',
            '.jfq-hbtn{background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.14);color:rgba(255,255,255,.7);border-radius:8px;padding:7px 14px;cursor:pointer;font-size:12px;font-weight:500;transition:all .15s;}',
            ...
            '#jfq-close{background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.18);color:rgba(255,255,255,.85);border-radius:50%;width:34px;height:34px;cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:1em;transition:background .2s;}',
            ...
            '.jfq-ep-num{font-size:10px;color:rgba(255,255,255,.35);width:40px;flex-shrink:0;font-family:monospace;font-weight:600;}',
            ...
            '.jfq-card-btn{position:absolute;top:15px;left:15px;right:auto;z-index:5;background:rgba(0,0,0,.7);border:1px solid rgba(255,255,255,.2);border-radius:4px;color:#fff;width:26px;height:26px;display:flex;align-items:center;justify-content:center;cursor:pointer;opacity:0;transition:opacity .15s,background .15s;backdrop-filter:blur(6px);}',
            ...
            '#jfq-detail-btn{display:inline-flex;align-items:center;justify-content:center;background:none;border:none;border-radius:50%;color:rgba(255,255,255,.76);width:auto;height:auto;padding:7px;cursor:pointer;transition:color .2s,background .2s;font-family:inherit;}',
            ...
            '#jfq-toast{position:fixed;bottom:28px;left:50%;transform:translateX(-50%) translateY(10px);background:rgba(15,15,20,.95);border:1px solid rgba(255,255,255,.14);backdrop-filter:blur(12px);color:#fff;padding:11px 22px;border-radius:10px;font-size:13px;z-index:9999999;opacity:0;transition:all .25s;pointer-events:none;white-space:nowrap;}',
    ...
    function renderSeasonPanel(panel, seriesId, seasons, activeIdx) {
        ...
        jfetch('/Users/'+uid()+'/Items?ParentId='+season.Id
            ...
            epList.innerHTML='';
            if(!eps.length){
                epList.innerHTML='<div style="padding:.5em 0;color:rgba(255,255,255,.25);font-size:.8em;">No episodes found.</div>';
                return;
            }
    ...
    function doSearch(q) {
        ...
        if(!q.trim()){
            res.innerHTML='<div class="jfq-hint">Search for a movie, series or episode<br>to add it to your queue.</div>';
            return;
        }
        ...
        jfetch('/Users/'+uid()+'/Items?SearchTerm='+encodeURIComponent(q.trim())
            ...
            items.forEach(function(item){
                ...
                var th=thumb(item);
                if(th){var img=document.createElement('img');img.src=th;img.onerror=function(){img.remove();};thumbDiv.appendChild(img);}
                ...
                if(item.Type==='Series'){
                    ...
                    expBtn.addEventListener('click',function(e){
                        ...
                        if(!isOpen){
                            ...
                            if(!expandPanel.dataset.loaded){expandPanel.dataset.loaded='1';loadSeasonsPanel(item.Id,expandPanel);}
                        }
                    });
                ...
                addBtn.textContent=isInQueue(item.Id)?'✓ Remove':(item.Type==='Series'?'+ All':'+ Add');
                addBtn.addEventListener('click',function(){
                    if(isInQueue(item.Id)){
                        removeById(item.Id);
                        addBtn.textContent=item.Type==='Series'?'+ All':'+ Add';
    ...
    function renderQueue() {
        ...
        if(!queue.length){
            ...
            queue.forEach(function(item,i){
                ...
                if(i>0){
                    var upB=document.createElement('button');upB.className='jfq-qb';upB.title='Move up';upB.textContent='↑';
                    upB.addEventListener('click',(function(idx){return function(){moveItem(idx,idx-1);};})(i));
                    acts.appendChild(upB);
                }
                if(i<queue.length-1){
                    var dnB=document.createElement('button');dnB.className='jfq-qb';dnB.title='Move down';dnB.textContent='↓';
                    dnB.addEventListener('click',(function(idx){return function(){moveItem(idx,idx+1);};})(i));
                    acts.appendChild(dnB);
                }
                var rmB=document.createElement('button');rmB.className='jfq-qb rm';rmB.title='Remove';rmB.textContent='✕';
                rmB.addEventListener('click',(function(idx){return function(){removeItem(idx);};})(i));
                acts.appendChild(rmB);
                row.appendChild(num);row.appendChild(td);row.appendChild(info);row.appendChild(acts);
        ...
        var shBtn=document.getElementById('jfq-shuffle-btn');
        var clBtn=document.getElementById('jfq-clear-btn');
    ...
    function openOverlay(){
        ...
        var inp=document.createElement('input');inp.type='text';inp.id='jfq-input';
        ...
        autoBtn.addEventListener('click',function(){
            ...
            function goToItem(){
                ...
                var t1=0,iv1=setInterval(function(){
                    t1++;
                    if((window.location.hash||'').indexOf(item.id)===-1){if(t1>50)clearInterval(iv1);return;}
                    clearInterval(iv1);
                    var t2=0,iv2=setInterval(function(){
                        t2++;var pb=null;
                        var cands=document.querySelectorAll('button.btnPlay,button[data-action="play"],button[data-action="resume"]');
                        for(var i=0;i<cands.length;i++){
                            var b=cands[i];
                            if(!b.classList.contains('hide')&&b.offsetParent!==null){pb=b;break;}
                        }
                        if(!pb){if(t2>40)clearInterval(iv2);return;}
                        clearInterval(iv2);pb.click();
                        var t3=0,iv3=setInterval(function(){
                            t3++;var vid=document.querySelector('video');
                            if(!vid){if(t3>50)clearInterval(iv3);return;}
                            clearInterval(iv3);var done=false;
                            function onDone(){
                                if(done)return;done=true;clearInterval(iv4);
                                queue.shift();saveQueue();renderQueue();
                                setTimeout(goToItem,1200);
                            }
                            if(window.Events&&window.playbackManager){
                                function onStop(e,info){
                                    if(info&&info.mediaInfo){
                                        window.Events.off(window.playbackManager,'playbackstop',onStop);
                                        onDone();
                                    }
                                }
                                window.Events.on(window.playbackManager,'playbackstop',onStop);
                            }
                            vid.addEventListener('ended',onDone,{once:true});
                            var lh=window.location.hash,iv4=setInterval(function(){
                                var nh=window.location.hash;
                                if(nh!==lh&&!document.querySelector('video'))onDone();
                                lh=nh;
                            },1500);
                        },300);
                    },250);
        ...
        manBtn.addEventListener('click',function(){
            ...
            closeOverlay();
            setTimeout(function(){
                if(window.appRouter&&appRouter.showItem){appRouter.showItem({Id:queue[0].id,ServerId:sid});return;}
    ...
    function injectCardButtons() {
        document.querySelectorAll('.card[data-id]:not([data-jfq-q])').forEach(function(card){
            ...
            btn.title=isInQueue(id)?'Remove from queue':'Add to queue';
    ...
    function injectDetailButton() {
        ...
        btn.addEventListener('click', function(e) {
            e.stopPropagation();
            if (isInQueue(itemId)) { removeById(itemId);updateBtn();return; }
