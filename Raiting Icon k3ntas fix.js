...
(function () {
  ...
  function renderRanking(tab){var b=gb();if(!b)return;var items=DC[tab];if(!items){b.innerHTML='<div class="jrspin">Loading…</div>';loadRanking(tab==='movies'?'Movie':'Series').then(function(r){DC[tab]=r;cset(tab,r);renderRanking(tab);});return;}b.innerHTML='';var sec=document.createElement('div');sec.className='jrs';sec.innerHTML='<h2>'+(tab==='movies'?'Ranked Movies':'Ranked Series')+'</h2><div class="jrsub">'+items.length+' rated title'+(items.length!==1?'s':'')+'</div>';if(!items.length){sec.innerHTML+='<div class="jrempty">No rated titles yet.</div>';b.appendChild(sec);return;}var list=document.createElement('div');list.className='jrl';items.forEach(function(item,i){var rank=i+1,medal=rank===1?'🥇':rank===2?'🥈':rank===3?'🥉':null;var img=item.imgTag?srv()+'/Items/'+item.id+'/Images/Primary?tag='+item.imgTag+'&maxHeight=110&quality=85':'';if(rank===4&&items.length>3){var dv=document.createElement('div');dv.className='jrdiv';list.appendChild(dv);}var wrap=document.createElement('div');wrap.className='jriw';wrap.dataset.name=item.name.toLowerCase();wrap.dataset.itemid=item.id;var row=document.createElement('div');row.className='jrrow'+(rank<=3?' hi':'');row.innerHTML='<div class="jrrank'+(medal?'':' pl')+'">'+(medal||rank)+'</div>'+(img?'<img class="jrposter" src="'+img+'" alt="" loading="lazy" onerror="this.style.display=\'none\'">':'<div class="jrposter" style="display:flex;align-items:center;justify-content:center;font-size:12px;color:rgba(255,255,255,.1)">▪</div>')+'<div class="jrinfo"><div class="jrname">'+esc(item.name)+'</div><div class="jrmeta">'+item.year+(item.genres?' · '+esc(item.genres):'')+'</div></div>'+'<div class="jrsc2"><div class="jravg">'+item.avg.toFixed(1)+'<small>/10</small></div><div class="jrstars">'+sts(item.avg)+'</div><div class="jrrc">'+item.count+' rating'+(item.count!==1?'s':'')+'</div></div>'+'<button class="jrpb">View All</button><button class="jrrb">⭐ Rate</button><button class="jrexp">▾</button>';var rb=row.querySelector('.jrrb'),pb=row.querySelector('.jrpb'),eb=row.querySelector('.jrexp'),pel=document.createElement('div');pb.addEventListener('click',function(e){e.stopPropagation();showViewAllPopup({Id:item.id,Name:item.name});});attachRate(rb,pel,item.id,item.name);var panel=document.createElement('div');panel.className='jrep';panel.style.display='none';panel.innerHTML='<div class="jrspin" style="padding:.7em 0">Loading…</div>';eb.addEventListener('click',function(e){e.stopPropagation();var op=panel.style.display!=='none';panel.style.display=op?'none':'block';eb.textContent=op?'▾':'▴';eb.classList.toggle('open',!op);if(!op&&!panel.dataset.loaded){panel.dataset.loaded='1';rget('/Ratings/Items/'+item.id+'/DetailedRatings').then(function(d){renderExpand(panel,d);});}});row.addEventListener('click',function(e){if(eb.contains(e.target)||rb.contains(e.target)||pb.contains(e.target))return;navTo(item.id);});wrap.appendChild(row);wrap.appendChild(pel);wrap.appendChild(panel);list.appendChild(wrap);});sec.appendChild(list);b.appendChild(sec);}
  var userNameCache={};
  ...
  function showViewAllPopup(item){
    var old=document.getElementById('jrva-overlay');if(old)old.remove();
    var ov=document.createElement('div');ov.id='jrva-overlay';
    var box=document.createElement('div');box.id='jrva-box';
    var head=document.createElement('div');head.id='jrva-head';
    var ttl=document.createElement('div');ttl.id='jrva-title';ttl.textContent='All Ratings — '+esc(item.Name||'');
    var cls=document.createElement('button');cls.id='jrva-close';cls.textContent='✕';
    cls.onclick=function(){ov.remove();};
    head.appendChild(ttl);head.appendChild(cls);
    box.appendChild(head);
    var list=document.createElement('div');list.innerHTML='<div class="jrspin" style="padding:1.5em 0">Loading…</div>';
    box.appendChild(list);
    ov.appendChild(box);
    ov.addEventListener('click',function(e){if(e.target===ov)ov.remove();});
    document.addEventListener('keydown',function jrvaEsc(e){if(e.key==='Escape'){ov.remove();document.removeEventListener('keydown',jrvaEsc);}});
    document.body.appendChild(ov);
    rget('/Ratings/Items/'+item.Id+'/DetailedRatings').then(function(data){
      var rows=data&&(Array.isArray(data)?data:(data.Ratings||data.ratings||[]));
      list.innerHTML='';
      if(!rows||!rows.length){list.innerHTML='<div class="jrempty" style="padding:1em 0">No ratings yet.</div>';return;}
      rows.sort(function(a,b){return(b.Rating||b.rating||0)-(a.Rating||a.rating||0);});
      var me=uid();
      rows.forEach(function(r){
        var rid=r.UserId||r.userId||'';
        var rname=r.UserName||r.userName||r.Name||r.name||'User';
        var rval=parseFloat(r.Rating||r.rating||0);
        var isMe=rid===me;
        var row=document.createElement('div');row.className='jrva-row';
        var av=document.createElement('div');av.className='jrva-av';
        var img=document.createElement('img');img.src=avurl(rid);img.alt='';
        img.onerror=function(){av.textContent=ini(rname);};
        av.appendChild(img);
        var nm=document.createElement('span');nm.className='jrva-name';
        nm.textContent=rname;
        if(isMe){var you=document.createElement('span');you.className='jrva-you';you.textContent='(you)';nm.appendChild(you);}
        var st=document.createElement('span');st.className='jrva-stars';st.textContent=sts(rval);
        var sc=document.createElement('span');sc.className='jrva-score';sc.textContent=rval.toFixed(1)+'/10';
        row.appendChild(av);row.appendChild(nm);row.appendChild(st);row.appendChild(sc);
        if(isMe){
          var eb=document.createElement('button');eb.className='jrva-editbtn';eb.textContent='Edit';
          var reviewEl=document.createElement('div');
          eb.addEventListener('click',function(){
            if(reviewEl.style.display!=='none'){reviewEl.style.display='none';eb.textContent='Edit';return;}
            reviewEl.style.display='block';eb.textContent='Close';
            if(reviewEl.dataset.built)return;
            reviewEl.dataset.built='1';
            reviewEl.className='jrva-review';
            var sel=Math.round(rval)||0;
            var sarr=[];
            var spick=document.createElement('div');spick.className='jrva-stars-pick';
            var slbl=document.createElement('div');slbl.className='jrva-score-lbl';slbl.textContent=sel?sel+'/10':'–';
            for(var i=1;i<=10;i++){(function(v){var s=document.createElement('span');s.className='jrva-star'+(v<=sel?' lit':'');s.textContent='★';
              s.addEventListener('mouseenter',function(){sarr.forEach(function(x,j){x.classList.toggle('lit',j<v);});slbl.textContent=v+'/10';});
              s.addEventListener('mouseleave',function(){sarr.forEach(function(x,j){x.classList.toggle('lit',j<sel);});slbl.textContent=sel?sel+'/10':'–';});
              s.addEventListener('click',function(){sel=v;sarr.forEach(function(x,j){x.classList.toggle('lit',j<sel);});slbl.textContent=sel+'/10';});
              sarr.push(s);spick.appendChild(s);})(i);}
            var ta=document.createElement('textarea');ta.className='jrva-textarea';ta.placeholder='Share your thoughts about this title…';
            var msg=document.createElement('div');msg.className='jrva-msg';
            var acts=document.createElement('div');acts.className='jrva-actions';
            var sub=document.createElement('button');sub.className='jrva-submit';sub.textContent='Submit Rating';
            var can=document.createElement('button');can.className='jrva-cancel';can.textContent='Cancel';
            can.onclick=function(){reviewEl.style.display='none';eb.textContent='Edit';};
            sub.addEventListener('click',function(){
              if(!sel){msg.className='jrva-msg err';msg.textContent='Please select a rating.';return;}
              sub.disabled=true;sub.textContent='Saving…';
              rsubmit(item.Id,sel).then(function(ok){
                if(ok){
                  var rev=ta.value.trim();
                  var reviewDone=function(){msg.className='jrva-msg ok';msg.textContent='✓ Saved!';sc.textContent=sel.toFixed(1)+'/10';st.textContent=sts(sel);setTimeout(function(){reviewEl.style.display='none';eb.textContent='Edit';},1200);sub.disabled=false;sub.textContent='Submit Rating';};
                  if(rev){fetch(srv()+'/Ratings/Items/'+item.Id+'/Review',{method:'POST',headers:ahFull(),body:JSON.stringify({Review:rev,review:rev})}).then(reviewDone).catch(reviewDone);}
                  else reviewDone();
                  cdel('movies');cdel('series');DC.movies=null;DC.series=null;
                }else{msg.className='jrva-msg err';msg.textContent='Error saving.';sub.disabled=false;sub.textContent='Submit Rating';}
              });
            });
            acts.appendChild(can);acts.appendChild(sub);
            reviewEl.appendChild(spick);reviewEl.appendChild(slbl);reviewEl.appendChild(ta);reviewEl.appendChild(acts);reviewEl.appendChild(msg);
          });
  ...
  function renderRateResults(container,items){container.innerHTML='';if(!items.length){container.innerHTML='<div class="jrempty">No results.</div>';return;}items.forEach(function(item){var wrap=document.createElement('div');wrap.className='jriw';var img=item.ImageTags&&item.ImageTags.Primary?srv()+'/Items/'+item.Id+'/Images/Primary?tag='+item.ImageTags.Primary+'&maxHeight=110&quality=85':'';var sid='jfrs-'+item.Id;var row=document.createElement('div');row.className='jrrow';row.innerHTML=(img?'<img class="jrposter" src="'+img+'" alt="" loading="lazy" onerror="this.style.display=\'none\'">':'<div class="jrposter" style="display:flex;align-items:center;justify-content:center;font-size:12px;color:rgba(255,255,255,.1)">▪</div>')+'<div class="jrinfo"><div class="jrname">'+esc(item.Name||'')+'</div><div class="jrmeta">'+esc(String(item.ProductionYear||''))+' · '+(item.Type==='Series'?'Series':'Movie')+'</div></div>'+'<div id="'+sid+'" class="jrsc2" style="font-size:.58em;color:rgba(255,255,255,.25);">…</div>'+'<button class="jrpb">View All</button>'+'<button class="jrrb">⭐ Rate</button>';rget('/Ratings/Items/'+item.Id+'/Stats').then(function(st){var el=document.getElementById(sid);if(!el)return;if(st&&st.TotalRatings){var avg=parseFloat(st.AverageRating||0);el.innerHTML='<div class="jravg">'+avg.toFixed(1)+'<small>/10</small></div><div class="jrstars">'+sts(avg)+'</div><div class="jrrc">'+st.TotalRatings+' ratings</div>';}else el.innerHTML='<span style="font-size:.58em;color:rgba(255,255,255,.18)">–</span>';});var rb=row.querySelector('.jrrb'),pb=row.querySelector('.jrpb'),pel=document.createElement('div');pb.addEventListener('click',function(e){e.stopPropagation();showViewAllPopup(item);});rb.addEventListener('click',function(e){e.stopPropagation();if(pel.hasChildNodes()){pel.innerHTML='';return;}rb.textContent='…';rb.disabled=true;myRating(item.Id).then(function(r){rb.disabled=false;rb.textContent=r?'⭐ '+r+'/10':'⭐ Rate';rb.classList.toggle('rated',!!r);showPicker(pel,item.Id,item.Name,r,function(nv){pel.innerHTML='';rb.textContent=nv?'⭐ '+nv+'/10':'⭐ Rate';rb.classList.toggle('rated',!!nv);rget('/Ratings/Items/'+item.Id+'/Stats').then(function(st){var el=document.getElementById(sid);if(!el||!st||!st.TotalRatings)return;var avg=parseFloat(st.AverageRating||0);el.innerHTML='<div class="jravg">'+avg.toFixed(1)+'<small>/10</small></div><div class="jrstars">'+sts(avg)+'</div><div class="jrrc">'+st.TotalRatings+' ratings</div>';});cdel('movies');cdel('series');DC.movies=null;DC.series=null;});});});row.addEventListener('click',function(e){if(rb.contains(e.target)||pb.contains(e.target))return;navTo(item.Id);});wrap.appendChild(row);wrap.appendChild(pel);container.appendChild(wrap);});}
  ...
  /* ── AGGRESSIVE VIEW ALL INJECTOR ──
     Scans ALL .jrrow elements every 500ms and injects View All button if missing */
  function injectViewAllButtons(){
    document.querySelectorAll('#jrb .jrrow').forEach(function(row){
      if(row.querySelector('.jrpb'))return; /* already there */
      var rb=row.querySelector('.jrrb');
      if(!rb)return;
      var btn=document.createElement('button');
      btn.className='jrpb';
      btn.textContent='View All';
      btn.style.cssText='background:rgba(0,164,220,.18)!important;border:1px solid rgba(0,164,220,.5)!important;color:#00c8ff!important;border-radius:6px!important;padding:4px 10px!important;font-size:.72em!important;font-family:inherit!important;cursor:pointer!important;white-space:nowrap!important;flex-shrink:0!important;margin-right:4px!important;';
      /* Find item id from the wrap's dataset or from the expand button */
      var wrap=row.closest('.jriw');
      var itemId=wrap?wrap.dataset.itemid:null;
      var itemName=wrap?(wrap.dataset.name||''):'';
      /* Search & Rate rows use jfrs- id */
      if(!itemId){var sd=row.querySelector('[id^="jfrs-"]');if(sd)itemId=sd.id.replace('jfrs-','');}
      btn.addEventListener('click',function(e){
        e.stopPropagation();
        if(itemId&&window.showViewAllPopup)showViewAllPopup({Id:itemId,Name:itemName});
      });
      row.insertBefore(btn,rb);
    });
  }
  setInterval(injectViewAllButtons,500);
  new MutationObserver(injectViewAllButtons).observe(document.body,{childList:true,subtree:true});
