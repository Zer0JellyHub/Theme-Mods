...
(function () {
  ...
  const TEXT_MAP = {
    ...
    'tv networks': 'network',    'tv network': 'network',
  ...
  const HIDDEN_ALWAYS = new Set([
    'fernsehsender', 'networks', 'network', 'sender',
    'tv networks', 'tv network', 'broadcast', 'broadcast networks',
  ]);
  ...
  function updatePageClass() {
    ...
    document.body.classList.toggle('jf-subpage', isSubpage);
  }

  /* ─── Mobile: Content-Abstand = echte Header-Höhe ────────────── */
  function fixMobileOffset() {
    if (window.innerWidth > 700) return;
    const header = document.querySelector('.skinHeader');
    if (!header) return;
    const h = header.getBoundingClientRect().height;
    if (!h || h < 50) return;
    // Exakt den mainAnimatedPages Container anpassen
    const pages = document.querySelectorAll('.mainAnimatedPages, .mainAnimatedPage');
    pages.forEach(el => {
      // Nur Top-Level Container (nicht verschachtelte)
      if (!el.closest('.mainAnimatedPages') || el.classList.contains('mainAnimatedPages')) {
        el.style.setProperty('padding-top', h + 'px', 'important');
      }
    });
  }
  ...
  new MutationObserver(() => {
    if (location.href !== lastUrl) {
      ...
      processAll();
      setTimeout(fixMobileOffset, 400);
    }
  ...
  window.addEventListener('resize', () => { updatePageClass(); fixMobileOffset(); });

  processAll();
  setTimeout(processAll, 600);
  setTimeout(fixMobileOffset, 1200);
  setTimeout(fixMobileOffset, 2500);
